      ...
      if (res.statusCode == 200) {
        final body = jsonDecode(res.body);
          ...
          if (body is Map<String, dynamic> && body.containsKey('data') && body['data'] is List) {
            final list = body['data']; // Correctly unwraps the list
            setState(() => _items = List<dynamic>.from(list));
          } 
          // Also handles the case where the server might just send a plain list
          else if (body is List) {
            setState(() => _items = List<dynamic>.from(body));
          } 
          // If neither format matches, it shows the error
          else {
