...
class _ProfitReportPageState extends State<ProfitReportPage> {
  // ... (All existing state properties remain the same) ...
  bool _loading = false;
  DateTimeRange? _range;
  double _totalRevenue = 0.0;
  double _totalCost = 0.0;
  double _totalProfit = 0.0; // Net Cash Flow
  ...
  // ... (All fetch functions: _fetchData, _fetchSummary, _fetchTransactions remain the same) ...
  ...
  // ... (All filter/chart/UI helper functions remain the same) ...
  ...
  Future<void> _exportPDF() async {
    ...
    pdf.addPage(pw.MultiPage(
      build: (ctx) => [
        ...
        pw.Text('Net Cash Flow: ₱${_totalProfit.toStringAsFixed(2)}', style: bold),
        pw.Divider(height: 16),
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        ...
        action: SnackBarAction(label: 'Open', onPressed: () {
          // This requires a platform-specific file opener package,
          // but for now, just having the path is good.
        }),
      ));
    }
  }
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    _buildSummaryCard('Total Restock Costs', _totalCost, Colors.red),
                  ],
                ),
                Card(
