...
if($method === 'GET' && strpos($path, '/api/reports/inventory-summary') !== false){
    $r = $this->model->getInventorySummary();
    jsonResponse($r);
    return;
}
if($method === 'GET' && strpos($path, '/api/reports/financial-summary') !== false){
    $start = $_GET['start'] ?? date('Y-m-01'); $end = $_GET['end'] ?? date('Y-m-d');
    // Get product_id from query
    $product_id = $_GET['product_id'] ?? null;
    // Pass product_id to the model
    $r = $this->model->getFinancialSummary($start, $end, $product_id);
    jsonResponse($r);
    return;
}
