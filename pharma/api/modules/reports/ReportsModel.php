...
public function getFinancialSummary($start, $end, $product_id = null) {
    ...
    // 1. Get Total Revenue from sales
    ...
    if (!empty($product_id)) {
        // We must join sale_items to filter by product
    ...
    // 2. Get Total Cost from restocks
    // This is NOT filtered by product_id, as it's a total cost query
...
public function getInventorySummary($low_stock_threshold = 5, $expiry_days = 30, $recent_adjustments = 5) {
    ...
    $expStmt = $this->pdo->prepare('SELECT COUNT(*) as count FROM products WHERE expiry_date IS NOT NULL AND expiry_date <= DATE_ADD(CURDATE(), INTERVAL :d DAY)');
