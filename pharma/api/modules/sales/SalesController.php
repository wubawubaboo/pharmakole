...
require_once __DIR__.'/../../logger.php'; // <-- 1. INCLUDE THE LOGGER
...
class SalesController {
    ...
    public function __construct() {
        ...
        ActivityLogger::init($this->pdo); // <-- 2. INITIALIZE THE LOGGER
    }
    ...
    public function handle($method, $get, $post, $body){
        ...
        if($method === 'POST' && strpos($path, '/api/sales/create') !== false){
            ...
            // 3. Get cashier name from the payload (which is the user's full_name)
            ...
            $sale = [
                'cashier' => $cashier_name, // Use the name from the payload
                'total' => round($total, 2),
                'tax' => round($tax, 2),
            ...
            if ($sale_id) {
                // 4. Pass the cashier's name to the logger
