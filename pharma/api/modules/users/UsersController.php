...
public function handle($method, $get, $post, $body){
    ...
    if($method === 'POST' && strpos($path, '/api/users/login') !== false){
        ...
        // --- MODIFICATION START ---
        // This check prevents the "access array offset on null" warning.
        if (!is_array($data) || !isset($data['username']) || !isset($data['password'])) {
            jsonResponse(['error'=>'Invalid request or missing username/password.'], 400); // 400 Bad Request
            return;
        }
        // --- MODIFICATION END ---
        ...
        if($u){
            jsonResponse(['success'=>true,'user'=>$u],200);
        } else {
