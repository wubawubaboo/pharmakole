...
IF %ERRORLEVEL% NEQ 0 (
    ...
    type %LOG_PATH%
    echo -------------------------
    REM Optional: Delete the empty .sql file on failure
    del %FULL_PATH%
) ELSE (
    echo.
    echo Backup complete for: %TARGET_DIR%
    REM Delete the empty log file since there was no error
    del %LOG_PATH%
)
GOTO :EOF